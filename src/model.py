...
def make_1step_sched():
    noise_scheduler_1step = DDPMScheduler.from_pretrained("/home/yun/workspace/model/huggingface/sd-turbo", subfolder="scheduler")
...
class Difix(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_vae=4, mv_unet=True, timestep=999):
        super().__init__()
        ...
        unet_path = os.path.join(pretrained_path, "unet")
        vae_path = os.path.join(pretrained_path, "vae")
        vae_py_path = os.path.join(vae_path, "autoencoder_kl.py")
        # vae = AutoencoderKL.from_pretrained(vae_path)
        spec = importlib.util.spec_from_file_location("custom_autoencoder", vae_py_path)
        custom_autoencoder_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(custom_autoencoder_module)
        
        # 2. 从加载的模块中获取自定义的 AutoencoderKL 类
        CustomAutoencoderKL = custom_autoencoder_module.AutoencoderKL
        
        print(f"Successfully loaded custom class '{CustomAutoencoderKL.__name__}' from {vae_py_path}")
        # 3. 使用这个正确的、自定义的类来加载权重和配置
        #    这个类知道如何处理 LoRA 权重和 'gamma' 等特殊配置
        vae = CustomAutoencoderKL.from_pretrained(vae_path)
        ...
        print("Applying monkey-patch to VAE forward methods.")
        vae.encoder.forward = my_vae_encoder_fwd.__get__(vae.encoder, vae.encoder.__class__)
        vae.decoder.forward = my_vae_decoder_fwd.__get__(vae.decoder, vae.decoder.__class__)

        assert hasattr(vae.decoder, 'skip_conv_1'), "Loaded VAE does not have the custom 'skip_conv_1' layer. The model file may be incorrect."
        vae.decoder.ignore_skip = False
        ...
        # print("="*50)
        print("="*50)
        print("Difix model initialized successfully for inference.")
        print("="*50)
