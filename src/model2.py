...
def make_sched(timestep=999):
    noise_scheduler = DDPMScheduler.from_pretrained("/home/yun/workspace/model/huggingface/sd-turbo", subfolder="scheduler")
    noise_scheduler.set_timesteps(1, device="cuda")   # 只走一步
    noise_scheduler.timesteps = torch.tensor([timestep], device="cuda")  # 强制改成 [199]
...
class Difix(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_vae=4, lora_rank_unet=8, mv_unet=False, timestep=999):
        super().__init__()
        ...
        self.sched = make_sched(self.timestep)
        ...
        vae.decoder.ignore_skip = False                                      # False
        ...
        #     target_modules=["to_q", "to_k", "to_v", "to_out.0"]  # 常见 cross-attn 模块
        # )
        ...
        # self.timesteps = self.sched.timesteps.clone().to("cuda").long()  # 与 scheduler 完全一致
    ...
    def set_train(self, mode=None):
        """ mode: 'vae' 只训练 VAE LoRA, 'unet' 只训练 U-Net LoRA, 'both' 同时训练 """
        self.unet.train()
        self.vae.train()
        ...
        if mode == "vae" or mode == "both":
            for n, p in self.vae.named_parameters():
                if "lora" in n or "skip_conv" in n:
    ...
    def forward(self, x, timesteps=None, prompt=None, prompt_tokens=None):
        ...
        # UNet + Scheduler step
        # model_pred = self.unet(z, t, encoder_hidden_states=caption_enc).sample
